...
Latest source/release package in this repository: `v0.3.6`.
...
If HLAE prints `mirv_loadlibrary failed`, fully restart HLAE/CS2 and verify the path is correct. Current `v0.3.6` DLLs no longer depend on `MSVCP140.dll`, `VCRUNTIME140.dll`, the UCRT `api-ms-win-crt-*` DLLs, or a load-time `D3DCOMPILER_47.dll`, so a missing VC++ runtime should not block loading.

If the console prints `LoadLibraryA Ok.` followed by `SetUpView hook blocked by compatibility gate`, the DLL did load, but an older compatibility gate rejected hook installation while schema offsets were still resolving. Update to `v0.3.6` or newer.

If the DLL is loaded while a demo is already playing, `v0.3.6` no longer waits for schema resolution before installing the camera hook. That avoids the startup hang seen in some demo-load cases.
